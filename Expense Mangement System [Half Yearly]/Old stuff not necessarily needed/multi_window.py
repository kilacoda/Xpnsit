...
layouts.append(
    [
        [sg.Text('Some Timestamp', key='date'), sg.CalendarButton('+', target='date')],
        [sg.Button('Next')]
    ]
)

# New timestamp behavior (sets current time instead of 00:00:00)
# New icon behavior (tries to use window icon or default PySimpleGUI icon instead of tkinter icon)
layouts.append(
    [
        [sg.Text('Some Timestamp', key='date'), sg.CalendarButton('+', target='date')],
        [sg.Button('Next')]
...
layouts.append(
    [
        [sg.Text('Some Timestamp', key='date'), sg.CalendarButton(
            '+', target='date', Locale='fr')],
        [sg.Button('Next')]
    ]
)

# New optional format parameter
layouts.append(
    [
        [sg.Text('Some Timestamp', key='date'), sg.CalendarButton(
...
for layout in layouts:
    ...
    while True:             # Event Loop
        ...
        if event is None or event == 'Next':
            break
